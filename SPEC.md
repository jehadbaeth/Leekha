...
5. Dealer selection: from the second round on, the round belongs to the player who ate the most points in the previous round (the "dealer"). On a tie, the tied player who ate the K♣ takes it. The seat to the dealer's right leads the first trick, not the dealer; the dealer only "deals" (nominally, no mechanical effect online).
6. **Forced talyeekh strengthens the base rule.** The documented base game only forces a Leekha card out when a player is void of the led suit (see Section 1's "talyeekh" definition). The Idlib variant goes further: if a player holds the led suit's own Leekha card (10♦ on a diamond lead, Q♠ on a spade lead, K♣ on a club lead) and a strictly higher card of that suit is already on the trick, guaranteeing the Leekha would lose, that card must be played immediately, even if the player holds other cards of that suit they could have followed with instead. If nothing on the trick yet beats the Leekha, the player may hold it back and follow with a different card of the suit, since playing it now would win the trick rather than surrender it. Leading a suit that has a Leekha card in it therefore guarantees that card is surrendered eventually once it would lose, by the holder if they can follow, or by whoever is void and forced to dump cross suit regardless of winning odds. This does not change the void-side forced dump rule at all; it only removes the choice to "hide" a losing Leekha behind other cards of its own suit.
...
* **Forced talyeekh (Idlib addition)**: a player who holds the led suit's own Leekha card must play it immediately once a higher card of that suit already lies on the trick (so the Leekha would lose), even if other cards of that suit are available to follow with instead. If nothing yet beats it, the player may hold it back and follow with a different card of the suit.
...
13. **Dealer tiebreak cascade and the first deal.** The K♣ eater takes the round on a points tie. If the tied players do not include the K♣ eater, default cascade: the Q♠ eater among them, then the 10♦ eater, then the tied seat closest to the previous dealer's right. The first round of every match and every rematch belongs to whoever is dealt the 7 of hearts.
...
1. The first round of a match belongs to whoever is dealt the 7 of hearts; they are round 1's dealer and lead its first trick.
...
3. Consequence worth naming: the opening lead belongs to the seat to the biggest eater's right, not the eater themselves; the eater only nominally "deals". Idlib players treat that lead as a signaling channel (Section 5), since it advertises what the dealer's right-hand neighbor was just handed.
...
1. The dealer leads the first trick with any card.
2. Play proceeds anticlockwise; each player plays exactly one card per trick.
3. Following: a player holding one or more cards of the led suit must play one of them. Exception: if the led suit's own Leekha card (10♦ for diamonds, Q♠ for spades, K♣ for clubs) is among them and it would currently lose the trick, forced talyeekh (rule 3a) applies and removes the free choice.
...
* A Leekha card cannot be hidden behind other cards of its own suit once it would lose: a lead of its suit that is already beaten forces it out via rule 3a even while its holder has other legal followers, and a void holder is forced to dump it cross suit via rule 4 regardless of whether it would win (off suit cards never win, so a cross suit dump is always a guaranteed loss already). Before that point, the holder may legally hold the Leekha back and follow with a different card of the suit instead, since playing it would win the trick rather than surrender it.
...
7. **Eating big marks you, and hands your right-hand neighbor the microphone.** The biggest eater owns the next round as dealer, but the seat to their right opens it. Idlib table culture treats that opening lead as a legal signal, announcing that a Leekha was just passed rightward (into the eater's right-hand neighbor's hand) or advertising the suit its owner is shortening in order to dump a Leekha later. Bots do not roleplay table talk, but Section 13 exploits the structural side of it, and the K♣ tiebreak is one more reason the king is the card nobody wants.
...
function legalPlaysFor(hand: Card[], trick: TrickState, cfg: RulesConfig): Card[] {
  ...
  if (base.length === 0) {
    ...
    if (leekhaOfSuit.length > 0 && leekhaOfSuit[0].rank < winningRank(trick, led)) {
      base = leekhaOfSuit;                                       // forced talyeekh: only once it would lose
    }
  }
