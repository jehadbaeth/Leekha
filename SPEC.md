...
5. Dealer selection: from the second round on, the round belongs to the player who ate the most points in the previous round (the "dealer"). On a tie, the tied player who ate the K♣ takes it. The dealer leads the first trick personally; the seat to the dealer's right serves (physically deals) the cards, which has no mechanical effect online.
...
3. Consequence worth naming: the opening lead belongs to the biggest eater personally, so the punished player at least controls the next round's opening signal. Idlib players treat that lead as a signaling channel (Section 5).
...
7. **Eating big marks you, and hands you the microphone.** The biggest eater owns the next round and opens it personally. Idlib table culture treats that opening lead as a legal signal, announcing that a Leekha was just passed rightward or advertising the suit its owner is shortening in order to dump a Leekha later. Bots do not roleplay table talk, but Section 13 exploits the structural side of it, and the K♣ tiebreak is one more reason the king is the card nobody wants.
